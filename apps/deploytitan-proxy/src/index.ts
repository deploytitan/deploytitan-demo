...
async function commitMessage(
  ...
  const getRes = await fetch(
    `${apiBase}/contents/${path}?ref=${GITHUB_BRANCH}`,
    {
      headers: {
  ...
  const authorObj = {
    ...
    email:
      email ??
      `${author.toLowerCase().replace(/\s+/g, '.')}@demo.deploytitan.com`,
...
async function fetchAndBroadcastDeployments(
  singleClient?: Response,
): Promise<void> {
...
app.get('/health', (_req: Request, res: Response) => {
  res.json({ status: 'ok', service: 'deploytitan-proxy' });
});
...
app.post('/api/commit', (req: Request, res: Response) => {
  void (async () => {
    ...
    if (
      emailStr &&
      (emailStr.length > 100 || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailStr))
    ) {
  ...
  void (async () => {
    ...
    try {
      const routingRes = await fetch(
        `${DEPLOYTITAN_API_URL}/routing-config/${encodeURIComponent(CORE_SERVICE_NAME)}/${encodeURIComponent(ENVIRONMENT)}`,
        {
          headers: { Authorization: `Bearer ${DEPLOYTITAN_API_KEY}` },
          signal: AbortSignal.timeout(5000),
        },
      );
      if (!routingRes.ok) {
        res.status(routingRes.status).json(await routingRes.json());
        return;
      }
...
app.post('/api/cohort', (req: Request, res: Response) => {
  void (async () => {
    ...
    if (!body.defaultDeploymentId) {
      res.status(400).json({ error: 'defaultDeploymentId is required' });
      return;
    }
    if (!Array.isArray(body.cohorts)) {
      res.status(400).json({ error: 'cohorts must be an array' });
      return;
    }
    try {
      const upstream = await fetch(`${DEPLOYTITAN_API_URL}/cohort-routing`, {
        ...
        body: JSON.stringify({
          serviceName: CORE_SERVICE_NAME,
          environment: ENVIRONMENT,
          strategy: 'cohort',
          cohorts: body.cohorts.map((c, i) => ({
            cohortId: c.cohortId,
            deploymentId: c.deploymentId,
            priority: c.priority ?? body.cohorts!.length - i,
          })),
...
app.post('/api/traffic', (req: Request, res: Response) => {
  void (async () => {
    ...
    const body = req.body as {
      splits?: Array<{ deploymentId: string; percentage: number }>;
    };
...
server.listen(PORT, () => {
  console.log(`[deploytitan-proxy] listening on port ${PORT}`);
});

process.on('SIGTERM', () => {
  console.log('[deploytitan-proxy] SIGTERM received, shutting down...');
  for (const client of sseClients) {
    client.end();
  }
